...
STRINGS = {
    "en": {
        ...
        "step1": "1. Upload file", "no_file": "No file uploaded",
        "step2": "2. Analyze against Egyptian codes",
        "analyze_now": "Analyze now", "step3": "3. Result",
        "dl_txt": "Download TXT", "dl_pdf": "Download PDF",
        ...
        "project_name": "Project Name", "engineer": "Engineer",
        "supervisor": "Supervisor", "company": "Company",
    ...
    "ar": {
        ...
        "charts_sub": "مبنية من حلقة التعلم الذاتي.",
        ...
        "project_name": "اسم المشروع", "engineer": "المهندس",
        "supervisor": "المشرف", "company": "الشركة",
        "location": "الموقع", "date": "التاريخ",
        "category": "الفئة", "version": "الإصدار",
        "confidence": "الثقة", "description": "الوصف",
...
<style>
:root {
    --hubx-bg: #0b1020; --hubx-surface: #131a2f;
    --hubx-surface-2: #1a2342; --hubx-border: #2a3559;
    --hubx-text: #e8ecf7; --hubx-text-dim: #9aa5c4;
    --hubx-primary: #4f8cff; --hubx-primary-2: #6aa8ff;
    --hubx-accent: #22d3a6; --hubx-warn: #f6a623;
    --hubx-danger: #ef4a5e;
    --hubx-radius: 12px; --hubx-radius-sm: 8px;
...
.hubx-lang-btn.active {
    background: var(--hubx-primary) !important; color: white !important;
}
.hubx-card {
    background: var(--hubx-surface) !important;
    border: 1px solid var(--hubx-border) !important;
    border-radius: var(--hubx-radius) !important;
    box-shadow: var(--hubx-shadow); padding: 20px;
}
.hubx-title { font-size: 1.6rem; font-weight: 800;
...
.hubx-body h1 { font-size: 1.55rem; font-weight: 800;
                margin: 0.2em 0 0.5em 0; line-height: 1.25;
                letter-spacing: -0.3px; }
.hubx-body h2 { font-size: 1.2rem; font-weight: 700;
                margin: 0.9em 0 0.35em 0; line-height: 1.3;
                color: var(--hubx-primary-2); }
.hubx-body h3 { font-size: 1.02rem; font-weight: 700;
                margin: 0.7em 0 0.25em 0; line-height: 1.3; }
.hubx-body p  { font-size: 0.95rem; line-height: 1.6;
                margin: 0.4em 0; color: var(--hubx-text);
                text-align: left !important; }
.hubx-body ul, .hubx-body ol {
    margin: 0.3em 0 0.6em 0; padding-left: 0;
    list-style-position: inside;
}
.hubx-body li { font-size: 0.95rem; line-height: 1.55;
                margin: 0.15em 0; text-align: left !important;
                padding-left: 0; }
.hubx-body strong { font-weight: 700; color: #fff; }
.hubx-body table {
    display: block; overflow-x: auto; max-width: 100%; width: 100%;
    border-collapse: collapse; margin: 0.5em 0; font-size: 0.85rem;
...
.hubx-body th, .hubx-body td {
    ...
    word-break: break-word; min-width: 90px; vertical-align: top;
}
...
.hubx-body blockquote {
    ...
    padding-left: 12px; margin: 0.5em 0; color: var(--hubx-text-dim);
}
body.lang-ar .hubx-body p, body.lang-ar .hubx-body li,
body.lang-ar .hubx-body h1, body.lang-ar .hubx-body h2,
body.lang-ar .hubx-body h3 { text-align: right !important; }
...
body.lang-ar .hubx-body ul, body.lang-ar .hubx-body ol {
    padding-left: 0; padding-right: 0; list-style-position: inside;
}
...
.hubx-drawer-btn {
    ...
    min-height: 34px !important; line-height: 1.25 !important;
...
.hubx-paper {
    ...
    font-size: 12pt; line-height: 1.55; direction: ltr;
}
...
.hubx-paper .section h2 { font-size: 14pt;
...
@media (max-width: 768px) {
    ...
    .hubx-header { padding: 8px 10px !important;
                   flex-wrap: wrap !important; gap: 6px !important; }
    .hubx-header > .row { flex-wrap: wrap !important;
                          gap: 6px !important; width: 100% !important; }
    .hubx-brand { font-size: 1rem !important; }
    .hubx-nav { width: 100% !important; flex-wrap: wrap !important;
                gap: 2px !important;
                justify-content: space-between !important; }
    .hubx-nav a { padding: 6px 4px !important;
    ...
    .hubx-stat { min-width: 80px !important;
...
def _paper_html(title, category, version, body_md, meta=None):
    meta = meta or {}
    rtl = is_rtl()
...
def _paper_pdf_bytes(html_str, rtl):
    ...
    <style>
      ...
      .hubx-paper .meta td {{ padding:3mm 2mm; font-size:12pt;
                              vertical-align:top; }}
      .hubx-paper .meta .lbl {{ width:35%; font-weight:700; color:#222; }}
      .hubx-paper .meta .val {{ width:65%;
                                border-bottom:1px dotted #999; }}
      .hubx-paper .section {{ margin-top:10mm; }}
      .hubx-paper .section h2 {{ font-size:14pt;
                                 border-bottom:1.5px solid #111;
                                 padding-bottom:2mm;
                                 margin-bottom:4mm; color:#111; }}
...
def _paper_body_html(title, category, version, body_md, meta=None):
    meta = meta or {}
    rtl = is_rtl()
    rtl_cls = " rtl" if rtl else ""
    today = datetime.utcnow().date().isoformat()
...
def _open_paper_dialog(title, category, version, body_md, meta=None):
        ...
        with ui.card().classes("hubx-card w-full"):
            ...
            async def do_analyze():
                if not state.get("extracted"):
                    ...
                    return
                progress.style("opacity:1"); err_label.text = ""
                ...
                try:
                    save_check_report(
                        state["filename"], state["file_type"],
                        state["original_text"], result.get("issues", []),
...
def knowledge_page():
    ...
    selected = {"cat": app.storage.user.get("k_cat"), "_sig": None}
    last_item = app.storage.user.get("k_item")
    ...
    with ui.column().classes("w-full p-2 gap-3"):
        ...
        with ui.card().classes("hubx-card w-full"):
            ...
            async def do_search():
                ...
                if not q:
                    ui.notify("Type a question.", color="orange"); return
    ...
    def _badge_html(verified, flagged):
        parts = []
        if verified:
            parts.append(f'<span class="hubx-badge" '
                         f'style="background:rgba(34,211,166,0.18);'
                         f'color:#22d3a6">{t("verified_badge")}</span>')
        if flagged:
            parts.append(f'<span class="hubx-badge" '
                         f'style="background:rgba(239,74,94,0.18);'
                         f'color:#ef4a5e">{t("flagged_badge")}</span>')
        return "  ".join(parts)
    ...
    def show_item(kid):
        ...
        with reader_toolbar:
            ...
            if verified:
                ui.button(t("flag"), on_click=lambda: _toggle_flag(k[0])
                          ).classes("hubx-btn hubx-btn-warn")
            else:
                ui.button(t("verify"), on_click=lambda: _toggle_verify(k[0])
                          ).classes("hubx-btn hubx-btn-accent")

    def _toggle_verify(kid):
        try:
            verify_knowledge(kid)
            ui.notify("Verified", color="green")
            show_item(kid)
        except Exception as e:
            ui.notify(f"Verify failed: {e}", color="red")

    def _toggle_flag(kid):
        try:
            flag_knowledge(kid)
            clear_knowledge_flags(kid)
            ui.notify("Flagged", color="orange")
            show_item(kid)
        except Exception as e:
            ui.notify(f"Flag failed: {e}", color="red")
...
def templates_page():
    ...
    selected = {"cat": app.storage.user.get("t_cat"), "_sig": None}
    last_item = app.storage.user.get("t_item")
    ...
    def show_template(tid):
        ...
        with toolbar:
            ...
            if verified:
                ui.button(t("flag"), on_click=lambda: _toggle_flag(tmpl[0])
                          ).classes("hubx-btn hubx-btn-warn")
            else:
                ui.button(t("verify"), on_click=lambda: _toggle_verify(tmpl[0])
                          ).classes("hubx-btn hubx-btn-accent")

    def _toggle_verify(tid):
        try:
            verify_template(tid)
            ui.notify("Verified", color="green")
            show_template(tid)
...
def charts_page():
    ...
    with ui.column().classes("w-full max-w-7xl mx-auto p-4 gap-3"):
        ...
        def style_ax(ax):
            ...
            ax.title.set_fontsize(12); ax.title.set_fontweight("bold")
        ...
        def draw_cat(fig):
            ...
            ax.set_title("Knowledge by category"); style_ax(ax)
            fig.tight_layout()
        ...
        def draw_conf(fig):
            ...
            ax.set_title("Confidence distribution"); style_ax(ax)
            ax.tick_params(axis="x", labelrotation=45); fig.tight_layout()
        ...
        def draw_versions(fig):
            ...
            ax.set_title("Version distribution"); style_ax(ax)
            fig.tight_layout()
        ...
        def draw_runs(fig):
            ...
            ax.plot(cycles, refined, marker="s", color="#ef4a5e",
                    label="refined")
            ax.set_title("Learning activity"); style_ax(ax)
